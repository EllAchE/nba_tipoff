...
from src.rating_algorithms.common_data_processing import preMatchPredictionsNoBinning, beforeMatchPredictions, \
    addSummaryMathToAlgoSummary, runAlgoForSeason

# backlogtodo optimize trueskill, glicko etc. for rapid iteration
# backlogtodo refactor equations here to be generic
def runTSForSingleSeason(season, seasonCsv, winningBetThreshold):
    runAlgoForSeason(season, seasonCsv, ENVIRONMENT.PLAYER_TRUESKILL_DICT_PATH, ENVIRONMENT.TS_PREDICTION_SUMMARIES_PATH,
                     trueskillBeforeMatchPredictions, trueskillUpdateDataSingleTipoff, winningBetThreshold, columnAddAlgo=None, startFromBeginning=False)
...
def runTSForAllSeasons(seasons, winning_bet_threshold=ENVIRONMENT.TS_TIPOFF_ODDS_THRESHOLD):
    ...
    for season in seasons:
        runTSForSingleSeason(season, ENVIRONMENT.SEASON_CSV_UNFORMATTED_PATH.format(season),
...
def updateTrueSkillDictionaryFromLastGame():
    runTSForSingleSeason(ENVIRONMENT.CURRENT_SEASON, ENVIRONMENT.CURRENT_SEASON_CSV, ENVIRONMENT.PLAYER_TRUESKILL_DICT_PATH, winningBetThreshold=ENVIRONMENT.TS_TIPOFF_ODDS_THRESHOLD, startFromBeginning=False)
