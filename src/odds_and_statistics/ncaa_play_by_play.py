...
def getPlayerLastNameFromShotDescription(
        description: str):  # if this need to be fully generic then fetch the playerlast names and do a match on that
...
def _getSingleQuarterStatistics(shotsBeforeFirstScore: pd.DataFrame):
    ...
    for abc in shotsBeforeFirstScore.EVENTNUM:
        ...
        dataList.append({"shotIndex": shotIndex, "team": playerTeam, "player": player, "opponentTeam": opponentTeam,
                         "shotType": shotType})  # free throws should be considered a collective shot, not individual
        shotIndex += 1
    return dataList


def _getFirstShotStatistics(q1Shots: pd.DataFrame, q2Shots: pd.DataFrame, q3Shots: pd.DataFrame, q4Shots: pd.DataFrame,
                            bballRefCode: str):
    ...
    return {"gameCode": bballRefCode, "quarter1": q1Stats, "quarter2": q2Stats, "quarter3": q3Stats,
            "quarter4": q4Stats}
...
def getParticipatingTeamsFromId(id):  # (id: str) -> dict[str, str]:
    ...
    return {"home": homeTeamCity + ' ' + homeTeamName, "homeId": homeTeamId, "away": awayTeamCity + ' ' + awayTeamName,
            "awayId": awayTeamId}
...
def getTipoffLine(pbpDf: DataFrame, returnIndex: bool = False):
    ...
    print('Home Desc', tipoffContent.HOMEDESCRIPTION, 'Vis Desc', tipoffContent.VISITORDESCRIPTION, 'Neut Desc',
          tipoffContent.NEUTRALDESCRIPTION)
...
def getAllFirstPossessionStatisticsIncrementally(season):
    pass
    '''
    Folder Structure of raw data is ncaahoopR_data-master/YYYY-YY/pbp_logs/YYYY-MM-DD/dddddddddd.csv
    Only 2017-2018 and after has possession stats, so will probably ignore older data for now (college cycles anyways)
    '''
